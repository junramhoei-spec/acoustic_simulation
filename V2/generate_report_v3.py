...
# Chronological file mapping: 10 + 3 + 7 = 20
file_groups = [
    # Group 1: 7월 실측 세션 (10개)
    {
        "group_id": 1,
        "group_title": "1그룹: 7월 실측 세션 (10개)",
        "files": [
    ...
    {
        "group_id": 2,
        "group_title": "2그룹: 8월 4일 오전 사전 세션 (3개)",
        "files": [
    ...
    {
        "group_id": 3,
        "group_title": "3그룹: 8월 4일 오후 정밀 링 컵 세션 (7개)",
        "files": [
...
def draw_profile_plot(lab_gt, mask_gt, H_gt, pred_r, pred_h, title, save_path, true_z_mm=None, true_r_mm=None):
...
for g in file_groups:
    ...
    for fn in g["files"]:
        ...
        save_img = os.path.join(plot_dir, f'cup_{global_idx:02d}.png')
        t_str = f"#{global_idx:02d} {disp_name}\nH err: {err_h:.1f}mm | r err: {err_r:.1f}mm"
        draw_profile_plot(lab_gt, mask_gt, H_gt, pred_r, pred_h, t_str, save_img, true_z_mm=tz_mm, true_r_mm=tr_mm)
...
print(f"Evaluated {len(all_results)} cups across {len(file_groups)} chronological groups.")

# Save json results
res_v3_path = os.path.join(plot_dir, 'results_v3.json')
with open(res_v3_path, 'w', encoding='utf-8') as rf:
    json.dump({
        "all": all_results,
        "groups": group_results
    }, rf, ensure_ascii=False, indent=2)

print(f"Saved results_v3.json at {res_v3_path}")
